const CONFIG = {
    introTitle: 'Bae à!',
    introDesc: `Trái đất vốn lạ thường
    Mà sao Nam cứ đi nhầm đường
    Lạc vào tim anh lẻ loi
    Đằng sau chữ yêu đây là thương`,
    btnIntro: 'Lovee',
    title: 'Phải chăng anh đã yêu ngay từ cái nhìn đầu tiên 😙',
    desc: 'Phải chăng anh đã say ngay từ lúc thấy nụ cười ấy ',
    btnYes: 'Vẫn cứ là thích Nam <33',
    btnNo: 'Bố không thích mày',
    question:'Trên thế giới hơn 7 tỉ người mà sao anh lại tìm thấy Nam <3',
    btnReply: 'Gửi đén anh yêu của em <3',
    reply: 'Yêu thì yêu mà không yêu phải yêu <33333333',
    mess: 'Anh biết mà 🥰. Luv Nam nhiều nhiều 😘😘',
    messDesc: 'Tối nay 7h anh qua đón nhé bae.',
    btnAccept: 'Dạaaaaaa <3',
    messLink: 'http://m.me/datt.510' //link mess của các bạn. VD: https://m.me/nam.nodemy
}
#include <stdio.h>
	
	int main() {
	int day = 4; 
		switch(day) {
		case 1:
			printf("Chuc ban may man lan sau");
			break;
		case 2:
			printf("Voucher 20kd");
			break;
		case 3:
			printf("Giam gia 10% cho tat ca account");
			break;
		case 4:
			printf("The cao 20k");
			break;
		case 5:
			printf("Account bat ki");
			break;
		case 6:
			printf("Account sung vip");
			break;
		case 7:
			printf("Account 2k gold");
			break;
		case 8:
			printf("Account tan binh 20k-50k");
			break;
		case 9:
			printf("+10k vao tai khoan mua hang");
			break;
		case 10:
			printf("Chuc ban may man lan sau");
		}
		return 0;
.chevron {
  position: relative;
  display: block;
  height: 1152px; /*height should be double border*/
}

.chevron::before,
.chevron::after {
  position: absolute;
  display: block;
  content: "";
  border: 2048px solid transparent; /*adjust size*/
}
